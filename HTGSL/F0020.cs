...
namespace HTGSL
{
	public class F0020 : Form
	{
		...
		{
			ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof(F0020));
			...
			this.Text = "F0020 - Khai báo sản phẩm";
			base.Load += new EventHandler(this.F0020_Load);
		...
		private void LoadData()
		{
			try
			{
				...
				this.conn.Open();
				this.ds = new DataSet();
				this.strSQL = "SELECT [region_id],[region_name],[note],[product_name], [make_time], [price] FROM [dbo].[REGIONS]";
				this.da = new SqlDataAdapter(this.strSQL, this.conn);
				this.da.Fill(this.ds, "regions");
				this.c1TrueDBGrid1.DataSource = this.ds.Tables["regions"];
				this.conn.Close();
		...
		private void TrueDBGridFormat()
		{
			...
			this.c1TrueDBGrid1.Columns["region_name"].Caption = "Tên khu vực";
			this.c1TrueDBGrid1.Columns["Note"].Caption = "Ghi chú";
			this.c1TrueDBGrid1.Columns["product_name"].Caption = "Tên sản phẩm";
			this.c1TrueDBGrid1.Columns["make_time"].Caption = "Thời gian chế tạo";
			this.c1TrueDBGrid1.Columns["price"].Caption = "Đơn giá";
			this.c1TrueDBGrid1.Splits[0].DisplayColumns["region_name"].Width = 80;
			this.c1TrueDBGrid1.Splits[0].DisplayColumns["region_name"].Frozen = true;
			this.c1TrueDBGrid1.Splits[0].DisplayColumns["region_name"].Locked = true;
			this.c1TrueDBGrid1.Splits[0].DisplayColumns["region_name"].Style.BackColor = Color.LightGray;
			this.c1TrueDBGrid1.Splits[0].DisplayColumns["region_name"].AllowFocus = false;
			this.c1TrueDBGrid1.Splits[0].DisplayColumns["Note"].Width = 80;
			this.c1TrueDBGrid1.Splits[0].DisplayColumns["Note"].Merge = ColumnMergeEnum.Restricted;
			...
			this.c1TrueDBGrid1.Splits[0].DisplayColumns["make_time"].Width = 120;
			this.c1TrueDBGrid1.Splits[0].DisplayColumns["product_name"].Width = 250;
		}
		private void butSave_Click(object sender, EventArgs e)
		{
			try
			{
				...
				this.da.Update(this.ds.Tables["regions"]);
				this.conn.Close();
		...
		{
			this.c1TrueDBGrid1.Columns["region_name"].FooterText = this.c1TrueDBGrid1.RowCount.ToString();
		}
