...
namespace HTGSL
{
	public class F9993 : Form
	{
		...
		private void F9993_Load(object sender, EventArgs e)
		{
			try
			{
				this.LoadData();
		...
		private void LoadData()
		{
			...
			this.conn.Open();
			this.ds = new DataSet();
			this.strSql = "SELECT [service_id],[service_name] FROM  [dbo].[SERVICES]";
			this.da = new SqlDataAdapter(this.strSql, this.conn);
			this.da.Fill(this.ds, "services");
			this.c1TrueDBGrid1.DataSource = this.ds.Tables["services"];
			this.conn.Close();
		}
		private void TrueDBGridFormat()
		{
			...
			this.c1TrueDBGrid1.Splits[0].DisplayColumns["service_id"].Locked = true;
			this.c1TrueDBGrid1.Columns["service_id"].Caption = "Mã dịch vụ";
			this.c1TrueDBGrid1.Columns["service_name"].Caption = "Tên dịch vụ";
			this.c1TrueDBGrid1.Splits[0].DisplayColumns["service_name"].Width = 210;
			this.c1TrueDBGrid1.Splits[0].DisplayColumns["service_id"].Width = 130;
			this.CountFooterMaster();
		}
		private void CountFooterMaster()
		{
			this.c1TrueDBGrid1.Columns["service_id"].FooterText = this.c1TrueDBGrid1.RowCount.ToString();
		}
		private void butSave_Click(object sender, EventArgs e)
		{
			try
			{
				...
				this.da.Update(this.ds.Tables["services"]);
				this.conn.Close();
		...
		{
			ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof(F9993));
			...
			this.Text = "F9993 - Dịch vụ";
			base.Load += new EventHandler(this.F9993_Load);
